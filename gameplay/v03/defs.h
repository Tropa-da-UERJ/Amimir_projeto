...
typedef struct {
    char text[50];       // Texto que aparece no botão (ex: "JOGAR")
    SDL_Rect rect;       // Posição e tamanho do botão (área clicável)
    bool isHovered;      // Estado de mouse-over (para destaque)
    int value;           // Valor que a seleção deste botão representa (ex: 1 = JOGAR)
} Button;
